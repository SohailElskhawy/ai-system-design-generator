...
- **API Endpoints**: Recommend at least 6-8 realistic REST API endpoints. Ensure that the endpoint route paths use generic parameter placeholders (e.g. '/api/v1/interviews/:id/responses' or '/api/v1/interviews/{id}/evaluate') instead of hardcoding specific example IDs (like UUIDs) directly in the route path definition. Example IDs should only appear inside the requestBody and responseBody JSON payloads. For each endpoint, you MUST write realistic, fully-populated, well-formatted JSON mock strings in requestBody and responseBody (no empty objects/arrays or placeholder comments; provide actual realistic values).
